@if(count($cart))
    ...
    <button type="button" class="btn btn-primary btn-block mt-3"
        x-on:click="async () => { if (!confirm('Create this invoice?')) return; const win = window.open('about:blank', '_blank'); const id = await $wire.call('createInvoice'); id ? (win.location.href = '/desk/invoice/' + id) : win.close(); }">
