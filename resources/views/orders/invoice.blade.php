...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .cod-box { border: 2px solid #000; padding: 4px; margin: 6px 0; text-align: center; font-weight: bold; font-size: 13px; }
        ...
        @media print { .no-print { display: none !important; } body { width: 80mm; margin: 0; padding: 0; } }
...
<body>
    ...
    <div class="copy">
        ...
        <div class="line"></div>
        <div class="stamp">ORDER INVOICE</div>
        <div class="line"></div>
        ...
        @if($order->delivery_address)
            <div class="row"><span>Address:</span><span style="text-align:right;max-width:55mm;">{{ $order->delivery_address }}</span></div>
        @endif
        ...
        @if($order->isCod())
            <div class="cod-box">** COLLECT ON DELIVERY **<br>₦{{ number_format($order->total_amount, 2) }}</div>
        @else
            <div class="row"><span>Payment:</span><span class="bold">PAID ONLINE</span></div>
            @if($order->payment_reference)
                <div class="row"><span>Ref:</span><span>{{ $order->payment_reference }}</span></div>
            @endif
        @endif

        @if($order->dispatched_at && $order->delivery_person_name)
            <div class="line"></div>
            <div class="row"><span>Rider:</span><span class="bold">{{ $order->delivery_person_name }}</span></div>
            @if($order->delivery_person_phone)
                <div class="row"><span>Rider Tel:</span><span>{{ $order->delivery_person_phone }}</span></div>
            @endif
            <div class="row"><span>Dispatched:</span><span>{{ $order->dispatched_at->format('d/m/Y H:i') }}</span></div>
        @endif

        @if($order->note)
            <div class="line"></div>
            <div style="font-size:10px;"><span class="bold">Note:</span> {{ $order->note }}</div>
        @endif
