...
class ConsultationPricing
{
    ...
    /**
     * How long an unpaid online booking is held.
     *
     * A booking has to exist before Paystack can reference it, so abandoned
     * checkouts leave unpaid requests behind. After this long they stop
     * counting as live requests - staff are not chasing bookings nobody paid
     * for, and the record is still there if anyone asks.
     */
    public static function holdMinutes(): int
    {
        return max(5, (int) AppSetting::get('consult_hold_minutes', 60));
    }

    /** An unpaid online booking that has been waiting too long. */
    public static function hasLapsed(Appointment $appointment): bool
    {
        return $appointment->payment_status === 'pending'
            && $appointment->isSelfBooked()
            && $appointment->created_at?->lt(now()->subMinutes(static::holdMinutes()));
    }
}
